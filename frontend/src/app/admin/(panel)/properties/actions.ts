...
export async function createPropertyAction(formData: FormData) {
  ...
  if (!payload.title || !payload.address_line || !payload.city || payload.price <= 0) {
    redirect("/admin/properties?error=Title%2C%20price%2C%20address%20and%20city%20are%20required");
  }

  const response = await createAdminProperty(payload);

  if (!response.ok) {
    redirect(`/admin/properties?error=${encodeURIComponent(response.message ?? "Failed to create property.")}`);
  }

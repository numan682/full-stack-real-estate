...
const ALLOWED_REMOTE_SCRIPT_HOSTS = new Set([
  "maps.google.com",
  "maps.googleapis.com",
]);
...
function isSafeLocalScriptSource(scriptSource: string) {
  ...
  try {
    ...
    return scriptUrl.protocol === "https:" && ALLOWED_REMOTE_SCRIPT_HOSTS.has(scriptUrl.hostname);
  } catch {
    return false;
  }
}
    ...
    (async () => {
      ...
      for (const scriptSource of orderedScripts) {
        ...
        try {
          ...
          console.warn("[TemplateScripts] Skipping script:", scriptSource, error);
        }
      }
